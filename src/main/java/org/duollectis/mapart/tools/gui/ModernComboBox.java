...
public class ModernComboBox<T> extends JComboBox<T> {
	...
	private static Color bgHover() { return GuiApp.BG_CARD; }
	private static Color border() { return GuiApp.BORDER; }
	private static Color borderHover() { return GuiApp.ACCENT_BRIGHT; }
	private static Color text() { return GuiApp.TEXT; }
	private static Color textDim() { return GuiApp.TEXT_DIM; }
	private static Color selection() { return GuiApp.SELECTION_BG; }
	private static Color popupBg() { return GuiApp.BG_DEEP; }
		...
		JLabel label = new JLabel(title.toUpperCase()) {
			...
			protected void paintComponent(Graphics g) {
				...
				Color lineColor = new Color(
					Math.min(255, GuiApp.BORDER.getRed() + 60),
					Math.min(255, GuiApp.BORDER.getGreen() + 60),
					Math.min(255, GuiApp.BORDER.getBlue() + 60),
					200
				);
	...
	private static final class DescriptionTooltip {
		...
		DescriptionTooltip(Component owner) {
			...
			label.setForeground(GuiApp.TEXT);
			label.setBackground(new Color(28, 32, 48));
			label.setOpaque(true);
			label.setBorder(BorderFactory.createCompoundBorder(
				BorderFactory.createLineBorder(GuiApp.BORDER, 1),
