...
class UiAnimator {
	 ...
	 */
	static Timer animateFloat(float from, float to, int durationMs, Consumer<Float> onValue, Runnable onDone) {
		int[] elapsed = {0};
	 ...
	 * @return запущенный таймер
	 */
	static Timer hoverTransition(JComponent target, Color from, Color to, int durationMs, Consumer<Color> applyColor) {
	 ...
	 */
	static void paintRipple(Graphics2D g2, RippleState state, int w, int h) {
	 ...
	 */
	static RippleState startRipple(int x, int y, JComponent component) {
	 ...
	 * @return запущенный таймер
	 */
	static Timer pulse(JComponent target, Color baseColor, float amplitude, int periodMs, Consumer<Color> applyColor) {
	...
	static final class RippleState {
