...
public class ModernCheckBox extends JCheckBox {
	...
	private static Color bgChecked() { return GuiApp.theme.getAccent(); }

	private static Color bgUnchecked() { return GuiApp.theme.getBgInput(); }

	private static Color borderColor() { return GuiApp.theme.getBorder(); }

	private static Color textColor() { return GuiApp.theme.getText(); }
	...
	private class CheckIcon implements javax.swing.Icon {
		...
		private void drawAnimatedCheck(Graphics2D g2, int x, int y, float progress) {
			...
			Color checkColor = GuiApp.theme.getTextOnAccent();
