...
class MainWindowActions {
	...
	ImportOptions showImportOptionsDialog() {
		...
		addBlocksCheckBox.setForeground(GuiApp.TEXT);
		...
		orderLabel.setForeground(GuiApp.TEXT_DIM);
		orderLabel.setFont(new Font("SansSerif", Font.BOLD, 11));

		JPanel panel = new JPanel();
		panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
		panel.setBackground(GuiApp.BG_CARD);
	...
	private JButton buildOrderToggleButton(String label, boolean isXy, boolean[] xySelected) {
		JButton btn = new JButton(label) {
			...
			protected void paintComponent(Graphics g) {
				...
				Color bg = active ? GuiApp.ACCENT : GuiApp.BG_INPUT;
				Color fg = active ? GuiApp.BG_DEEP : GuiApp.TEXT;
				Color border = active ? GuiApp.ACCENT : GuiApp.BORDER;

				if (getModel().isRollover() && !active) {
					bg = new Color(GuiApp.BORDER.getRed(), GuiApp.BORDER.getGreen(), GuiApp.BORDER.getBlue());
				}
	...
	void onDitheringSuccess(Ditherer ditherer) {
		...
		w.progressBar.setForeground(GuiApp.SUCCESS);
	...
	void onConversionError(String message) {
		...
		w.progressBar.setForeground(GuiApp.ERROR);
	...
	void onConversionCancelled() {
		...
		w.progressBar.setForeground(GuiApp.WARN);
	...
	void setImportingState(boolean importing) {
		...
		if (importing) {
			...
			w.progressBar.setForeground(new Color(140, 100, 220));
		}
	}
	...
	void setConvertingState(boolean converting) {
		...
		if (converting) {
			...
			w.progressBar.setForeground(GuiApp.ACCENT);
	...
	private void switchConvertButtonToStop() {
		...
		w.convertButton.setBackground(GuiApp.ERROR);
	...
	void loadBlocksFromFile(File file) {
		try {
			...
			w.blocksCountLabel.setText(Lang.t("label.blocks_count", w.enabledBlocks.size()));
			w.blocksCountLabel.setForeground(GuiApp.SUCCESS);
	...
	void log(String message) {
		w.logArea.append(message + "\n");
		w.logArea.setCaretPosition(w.logArea.getDocument().getLength());
	}
	...
	void addHoverEffect(JButton btn) {
 		Color baseBg = btn.getBackground();
		...
		btn.addMouseListener(new MouseAdapter() {
			...
			public void mouseEntered(MouseEvent e) {
				...
				activeTimer = UiAnimator.animateFloat(from, 1f, 150, progress -> {
					...
					btn.setForeground(UiAnimator.lerp(GuiApp.TEXT_DIM, GuiApp.TEXT, progress));
					Color hoverBg = UiAnimator.lerp(baseBg, brighten(baseBg), progress);
