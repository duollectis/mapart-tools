...
class UiComponentFactory {
	...
	static JTextField buildTextField(String placeholder) {
		...
		field.setBackground(GuiApp.BG_INPUT);
		field.setForeground(GuiApp.TEXT);
		field.setCaretColor(GuiApp.ACCENT);
		field.setBorder(BorderFactory.createCompoundBorder(
			BorderFactory.createLineBorder(GuiApp.BORDER),
	...
	static JButton buildIconButton(String text, int fontSize, Insets padding) {
		...
		btn.setForeground(GuiApp.TEXT_DIM);
	...
	private static void addHoverEffect(JButton btn) {
		btn.addMouseListener(new MouseAdapter() {
			...
			public void mouseEntered(MouseEvent e) {
				stopPrevious();
				activeTimer = UiAnimator.hoverTransition(btn, GuiApp.TEXT_DIM, GuiApp.TEXT, 150, btn::setForeground);
			}

			@Override
			public void mouseExited(MouseEvent e) {
				stopPrevious();
				activeTimer = UiAnimator.hoverTransition(btn, GuiApp.TEXT, GuiApp.TEXT_DIM, 150, btn::setForeground);
			}
